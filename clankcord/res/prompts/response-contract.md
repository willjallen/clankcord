...
If you use a Clankcord command that writes or mutates state, submit a concise visible response after the command reports success when the command itself did not already deliver the requested user-visible result. Session lifecycle commands, automations, room controls, feedback, publication, transcript creation, reminders, and sound playback are state-changing actions.
