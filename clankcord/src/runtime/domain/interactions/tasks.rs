...
pub fn agent_master_prompt() -> String {
    [
        ...
        "If you publish a visible response, use `clankcord responses send` for the current session surface or `clankcord responses dm` for explicit DMs. You may submit multiple visible responses for one job when the answer naturally needs multiple messages. After successful submission, finish with RESPONSE_SUBMITTED. Final text is not a publication path.",
        "",
