...
use crate::runtime::jobs::{
    ...
    DiscordVoicePlayAudioOutput, DiscordVoicePlaybackOutput, JobCreatedOutput, JobMetadata,
...
struct PreV0_10_0ConfirmationJobMetadata {
    delivery: String,
    channel_id: String,
    message_id: String,
    post_error: String,
...
struct PreV0_10_0JobMetadata {
    detail: Option<Box<PreV0_10_0JobMetadataDetail>>,
    error: String,
    timed_out_at: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
struct PreV0_10_0DiscordVoiceLeaveOutput {
    session_id: String,
    status: String,
    ...
    audio_jobs: Vec<PreV0_10_0Job>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
struct PreV0_10_0DiscordVoiceStatusSnapshotOutput {
...
enum PreV0_10_0JobOutput {
    ...
    DiscordVoiceLeave(PreV0_10_0DiscordVoiceLeaveOutput),
    ...
    DiscordVoiceStatusSnapshot(PreV0_10_0DiscordVoiceStatusSnapshotOutput),
...
async fn reencode_job_payloads(
    ...
    for row in rows {
        ...
        let job = decode_pre_v0_10_0_job_payload_blob(&payload_blob).map_err(|error| {
            anyhow::anyhow!("migrating v0.10.0 job payload blob {job_id}: {error:#}")
        })?;
        sqlx::query(
            r#"
            UPDATE job_payloads
...
fn decode_pre_v0_10_0_job_payload_blob(bytes: &[u8]) -> Result<Job> {
    let body = pre_v0_10_0_envelope_body(bytes)?;
    if let Ok(job) = bincode::deserialize::<Job>(body) {
        return Ok(job);
    }
    let previous: PreV0_10_0Job = bincode::deserialize(body)?;
    previous.into_current()
}

fn pre_v0_10_0_envelope_body(bytes: &[u8]) -> Result<&[u8]> {
    ...
    {
        anyhow::bail!("job payload blob is not a pre-v0.10.0 encoded job payload");
    }
    let version_offset = JOB_PAYLOAD_BLOB_MAGIC.len();
    let version = u16::from_le_bytes([bytes[version_offset], bytes[version_offset + 1]]);
    if version != PRE_V0_10_0_JOB_PAYLOAD_BLOB_VERSION {
        anyhow::bail!(
            "unsupported pre-v0.10.0 job payload blob version {version}; expected {PRE_V0_10_0_JOB_PAYLOAD_BLOB_VERSION}"
        );
    }
    Ok(&bytes[header_len..])
}
...
impl PreV0_10_0JobMetadata {
    fn into_current(self) -> Result<JobMetadata> {
        let mut metadata = JobMetadata {
            ...
            output: self
                .output
                .map(PreV0_10_0JobOutput::into_current)
                .transpose()?,
            ..JobMetadata::default()
        };
        match self.detail.map(|detail| *detail) {
            Some(PreV0_10_0JobMetadataDetail::AgentTask(task)) => {
                metadata.set_agent_task(task);
            }
            Some(PreV0_10_0JobMetadataDetail::Confirmation(confirmation)) => {
    ...
    fn into_current(self) -> Result<crate::runtime::jobs::DiscordVoiceLeaveOutput> {
        Ok(crate::runtime::jobs::DiscordVoiceLeaveOutput {
            ...
            audio_jobs: self
                .audio_jobs
                .into_iter()
                .map(PreV0_10_0Job::into_current)
                .collect::<Result<Vec<_>>>()?,
        })
    }
}

impl PreV0_10_0JobOutput {
