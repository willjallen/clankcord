...
Dedicated Discord voice bot tokens form the capture pool. Each bot reports `VoiceBotStatus` with readiness, Discord user identity, current Discord location, pending disconnect state, gateway status, receive backend, and the latest adapter error. `VoiceBotStatus` describes the bot process and observed Discord state. A pending disconnect timestamp keeps a bot out of assignment selection and orphan cleanup while a completed Discord leave is settling through gateway observation.
...
Voice status sync reconciles adapter memory with Discord voice state before writing durable bot and session status. A ready bot that is not joining is checked against Discord for its current guild voice state, using the bot's observed guild or the configured guilds when adapter memory has no guild. The adapter snapshot includes the current non-bot voice states and observed guild IDs from configured guild caches. Runtime sync applies that snapshot to the durable voice-state table, recording ordinary leave transitions for persisted users that are absent from the observed guild snapshot. When Discord reports that the bot has no voice state, the adapter clears the bot location, finishes the live capture session, and submits final audio segment jobs. The runtime sync then closes capture sessions, capture runs, and assignments that no longer have a matching bot location and capture session.
...
An assigned voice bot leaves an empty room after the room has had no human participants for longer than `pool.auto_leave_empty_seconds`. This empty-room rule applies to every assignment. Automatic placement also releases a room with one human participant when `pool.auto_leave_single_deafened_seconds` is greater than zero and that participant has been deafened for at least that duration. A value of `0` disables the single-deafened release policy. A room released by automatic policy receives an auto-join suppression marker for `pool.auto_rejoin_cooldown_seconds` before policy can place a bot there again.

Placement also clears Discord voice bot presence that has no active assignment or capture session. That orphan cleanup issues one scoped Discord leave for each orphaned guild/channel pair, records reason `orphan_voice_bot_presence`, and does not depend on the channel being a configured room. The adapter removes any local Songbird call for the selected bot, reads the bot member's Discord voice state through the control bot token, disconnects that member through the same control token when Discord reports an active voice state, verifies that Discord no longer reports the bot in voice, and then persists a pending disconnect marker in `VoiceBotStatus`.
