...
The room placement automation reads configured rooms, live human voice-state rows, room controls, active assignments, active capture sessions, and voice bot status. It emits `room_agent_placement` jobs for automatic joins and releases, and emits direct `discord_voice_leave` jobs for orphaned voice bot presence grouped by guild/channel. A room with `auto_join = true` receives an available voice bot when pool auto-join is enabled and its live human participant count reaches `pool.auto_join_min_participants`. An available bot is ready, has no active assignment, has no current Discord voice channel, and has no pending disconnect. A room with an assigned bot releases after it remains empty longer than `pool.auto_leave_empty_seconds`. A room with one human participant releases when `pool.auto_leave_single_deafened_seconds` is greater than zero and that participant has been deafened for at least that duration; `0` disables that policy. Orphan voice bot presence is ignored while the bot has a pending disconnect marker.
